...
class Evaluation:
    ...
    def input(self):
        ...
        for key in self.competencies.keys():
            ...
            self.competencies[key][1] = int(input(f"Enter weightage({100 - weightTotalc} remaining): "))
            weightTotalc += self.competencies[key][1]
        weightTotalp = 0
        for key in self.performance.keys():
            self.performance[key][0] = int(input(f"Enter rating for {key}: "))
            self.performance[key][1] = int(input(f"Enter weightage({100 - weightTotalp} remaining): "))
            weightTotalp += self.performance[key][1]
